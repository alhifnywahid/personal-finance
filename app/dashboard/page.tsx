  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const totalDebt = debtSnapshot.docs.reduce(
          (sum, doc) => sum + doc.data().amount,
          0
        );
        ...
        const totalCredit = creditSnapshot.docs.reduce(
          (sum, doc) => sum + doc.data().amount,
          0
        );
        ...
        const recentTransactionsSnapshot = await getDocs(
          recentTransactionsQuery
        );
        const recentTransactionsData = recentTransactionsSnapshot.docs.map(
          (doc) => ({
            id: doc.id,
            ...doc.data(),
          })
